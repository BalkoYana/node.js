...
async function find({ searchString = '', page = 1, perPage = Number.MAX_SAFE_INTEGER }) {
    ...
    const searchResult = mockData.apartments.filter(u => u.district?.toLowerCase().includes(searchString));
...
async function findById(id) {
    return mockData.apartments.find(u => u.id == id);
}

async function update(apartmentId, apartmentData) {
    const index = mockData.apartments.findIndex(u => u.id === apartmentId);
...
async function remove(id) {
    mockData.apartments = mockData.apartments.filter(u => u.id != id);
};
